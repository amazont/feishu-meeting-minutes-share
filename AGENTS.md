# AGENTS.md — 给 Codex 的执行手册

你的任务:帮用户把「每日飞书会议纪要」自动化跑起来。这是一个把当天飞书会议录音(妙记)
自动整理成 8 维度纪要、归档到飞书知识库并通知的服务。本目录已含全部脚本,**你主要是按序执行 + 验证 + 出错时引导用户授权**。
...
- 涉及**运行器登录**(`Codex`/`sc Codex`)你无法替用户完成,要把命令给用户、让他自己跑。
- **飞书授权(`lark-cli auth login`)不要让用户敲命令**——你自己走 Device Flow 把二维码递给用户,见下方「飞书授权:Device Flow 标准流程」。
- 不要把任何 token / open_id 打印到对外渠道。

---

## 飞书授权:Device Flow 标准流程(token 缺失/失效时直接执行,不要先问)

凡遇到:首次安装未授权、`auth-healthcheck.sh` 告警 `needs_refresh`、运行报 `131006`/建知识库节点失败——**立即按以下四步执行**,把二维码直接呈现给用户,不要把交互式命令甩给用户:

```bash
# ① 发起(返回 verification_url + device_code;device_code 10 分钟过期,过期重新发起,不要缓存)
lark-cli auth login --no-wait --json --scope "contact:user.base:readonly minutes:minutes.search:read minutes:minutes.artifacts:read minutes:minutes.transcript:export wiki:node:retrieve wiki:node:create docx:document:create im:message.send_as_user drive:drive.metadata:readonly vc:note:read"

# ② 生成二维码(--output 只接受相对路径,先 cd 到目标目录)
cd /tmp && lark-cli auth qrcode "<verification_url>" --output ./auth-qr.png
```

③ 把**二维码图片 + verification_url 链接**一起展示给用户(URL 原样输出,不得改写/编码),告知「扫码或点链接授权后回来说一声」,然后结束本轮等用户。

```bash
# ④ 用户确认授权后,由你执行轮询完成,并跑自检验证恢复
lark-cli auth login --device-code "<device_code>"
bash auth-healthcheck.sh   # 静默 = 正常
```

> 远程部署(headless 机器)同样适用:整个流程都是非交互命令,经 ssh 跑即可;二维码 PNG 拉回本地展示。
...
```bash
command -v lark-cli && (command -v sc || command -v Codex)
```

- 缺 `lark-cli` → 让用户安装:`npm i -g @larksuite/cli` 然后 `lark-cli config init`。
- 缺 `sc` 和 `Codex` → 让用户安装 Codex(或 stepcode `sc`)。
...
```
- 有输出 → 已授权,继续。
- 无输出 → **按上方「Device Flow 标准流程」由你完成授权**(不要让用户手敲交互式 login)。
...
`init.sh` 会**自动**:取 open_id、解析个人知识库 space_id、创建/复用「会议纪要」根节点、探测飞书域名、写 `config.sh`、安装 workflow 到 `~/.Codex/workflows/`,并在最后**询问运行频率(默认每天一次 18:47)后自动装好定时任务**(macOS launchd / Linux cron)。

- 频率选择是交互式的(菜单 1-4):直接回车 = 每天一次。若你替用户跑 init 处于非交互环境,会自动用默认(每天 18:47)。
- 成功标志:打印一串 `✅`、显示「自动运行频率:…」并以 `🎉 初始化完成` 结尾,且生成了 `config.sh`。
- 若报「拿不到 open_id / 解析失败」→ 是授权没给全,按「Device Flow 标准流程」用它打印的 scope 补授权,然后重跑 `./init.sh`。
...
- 日志含 `Not logged in` → 让用户跑一次 `Codex`(或 `sc Codex`)登录,再重试。
...
| 现象 | 处理 |
|---|---|
| init 报拿不到 open_id / 解析失败 | 授权不全 → 按「Device Flow 标准流程」用它打印的 scope 补授权后重跑 init |
| run 日志含 `Not logged in` | 让用户跑 `Codex` / `sc Codex` 登录一次 |
...
- `run.sh` — 入口脚本:依赖/PATH 自检 + 确定性建节点 + 调 workflow + 失败告警(已修 set -e 死代码)+ 读 `_result.json` 写台账/state + 拉回备份 + 日志轮转。自动识别 `sc Codex` 或 `Codex`。
