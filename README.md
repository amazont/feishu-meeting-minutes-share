...
把当天所有飞书会议录音(妙记)自动整理成多维度纪要(其中**总结/一句话结论**以可视化「核心结论看板」画板呈现),
归档到你飞书知识库的「会议纪要 / <日期>」下，并发飞书通知。可设成每天自动跑。

## 纪要长什么样（8 维度）

每篇按固定顺序输出 8 个部分（其中 **会议背景 / 核心结论 / 后续 To Do** 为较新加入）：

1. **会议背景** — 为什么开这个会、要解决的问题（取不到则中性概述，不臆造）
2. **总结 / 一句话结论** — 一张「核心结论看板」**画板**(内联 SVG 原生画板:结论高亮带 + 2–4 张方向/判断卡片 +(有阶段时)里程碑时间线),建档后即为可编辑的飞书画板,替代原先的 2–4 句纯文字
3. **智能章节** — 按话题分段，加粗小标题 + 起止时间码 + 独立要点段
4. **核心结论** — 会上达成的关键结论 / 共识（3–6 条要点）
5. **关键决策** — `决策事项 / 负责人 / 时间节点` 三列表格
6. **后续 To Do** — `负责人 / 待办事项` 行动项表格（区别于「决策」：这里是"接下来谁去做什么"）
7. **金句时刻** — 代表性发言引用（含说话人）
8. **相关链接** — 飞书妙记原文链接
