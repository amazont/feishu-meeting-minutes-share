export const meta = {
  ...
  description: '把当日飞书妙记自动生成 8 维度会议纪要(背景/总结/章节/核心结论/决策/ToDo/金句/链接),独立 checker 打分校验(不达标自动重写),按天归档知识库并通知',
  whenToUse: '每天定时或手动触发,汇总当天所有飞书妙记为会议纪要文档,按天分目录归档',
  phases: [
    { title: 'Discover', detail: 'minutes +search 拉取今日妙记,读 ledger 去重(跳过已 DONE)' },
    { title: 'Draft', detail: '每篇取 notes/逐字稿 → 生成 8 维度 markdown 并写本地' },
...
const CRITERIA = `C1 覆盖度:八个二级标题(会议背景/总结/智能章节/核心结论/关键决策/后续 To Do/金句时刻/相关链接)齐全且非空。
C2 正确性:背景/总结/章节/核心结论/ToDo 忠实于源妙记(notes/逐字稿),不臆造决策、结论、待办或金句。
C3 可验证性:相关链接指向有效 minute URL;标题为概括内容的 12-20 字,不是「某某的视频会议」这类默认名。
C4 一致性/排版:Lark-flavored Markdown —— 首行 \`# 标题\`、一个 <callout> 会议信息块(每行单个 <br/> 分隔、不用 <br/><br/>)、块之间有空行;尤其「三、智能章节」必须是「**加粗编号标题行** + 独立要点段」的结构,不得把「标题+时间+正文」挤进单个列表项、不得用有序列表 \`1.\` 承载长正文;「五、关键决策」与「六、后续 To Do」是表格且单元格不含 <br/> 与 @用户/<cite> 提及标记;「一、会议背景」「四、核心结论」忠实于源、不臆造。飞书能正确渲染。`
...
3. 生成 Lark-flavored Markdown(严格按下面结构,否则飞书渲染会糊;每个块之间都要有空行):
   ...
   - 随后八个二级标题,顺序固定,各自排版如下:

     ## 一、会议背景
     —— 1–2 句交代为什么开这个会:触发点、要解决的问题或上下文。源里有就提炼,确实无背景信息就写一句中性概述,不要臆造。

     ## 二、总结
     —— 2–4 句概述写成一段;若有多条主线,改用无序列表,每条「- 」一行一句,别堆成长句。

     ## 三、智能章节  【重点,务必照此排版,这是最容易写糊的地方】
     ...
     ## 四、核心结论
     —— 提炼会上达成的关键结论/共识(是"得出什么判断、定了什么基调"层面,不是逐条决策),3–6 条无序列表,每条「- 」一句话说清。确无明显结论写一行「本次以信息同步为主,无强结论」。

     ## 五、关键决策
     —— 三列 markdown 表格:| 决策事项 | 负责人 | 时间节点 |。每行一条决策;单元格内不要换行、不要放 <br/>;负责人写纯姓名文本,不要嵌入 @用户 / <cite> 提及标记;若无明确决策,写一行「本次无明确决策事项」。

     ## 六、后续 To Do
     —— 两列 markdown 表格:| 负责人 | 待办事项 |。一行一个责任人(同人多事在"待办事项"列用顿号分隔,不要放 <br/>);区别于「关键决策」——这里是"接下来谁去做什么"的行动项;负责人写纯姓名文本,不嵌 @用户/<cite>;无明确待办写一行「本次无明确后续事项」。
